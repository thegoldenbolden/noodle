...
export const run: Params = async (interaction, autorole, channel, roles, message) => {
 ...
 if (roles.map((r) => r?.id).some((r: any) => autorole.role_ids.includes(r))) {
  throw new PastaError({ message: `A role provided is already being used.` });
 }
 ...
 if (autorole.type == "reaction") {
  if (parsedEmojis?.success.map((e) => e.id ?? e.name).some((e: any) => autorole.emoji_ids?.includes(e))) {
   throw new PastaError({ message: "A provided emoji is already being used." });
  }
 ...
 if (autorole.type == "button") {
  ...
  if (components.length > 25) throw new PastaError({ message: "There can only be 25 buttons on a message." });
 ...
 if (autorole.type == "menu") {
  ...
  if (opts.length > 25) throw new PastaError({ message: "There can only 25 roles on a menu." });

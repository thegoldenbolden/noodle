...
export const run: Params = async (interaction, title, type, channel, roles) => {
	...
	if (parsedEmojis?.failed?.[0]) {
			throw new PastaError({ message: `We cannot use the following emojis: ${parsedEmojis.failed.join(", ")}`});
	}
			...
			emojis.forEach(async (e: any, i: number) => {
					const timeout = setTimeout(async () => {
							try {
									...
									handleError(err as any, interaction);
							}
					}, 2000);
			});
			return;
...
export async function parseEmojis(interaction: I, roles: R, type: Type): Promise<{ success: any[]; failed: any[] }> {
	if (!roles || roles.size === 0) throw new PastaError({ message: `We are unable to use the roles provided.`});
...
export function menu(roles: R) {
	if (!roles || roles.size === 0) throw new PastaError({ message: `We are unable to use the roles provided for the menu.` });
	return {
...
export function createButtons(emojis: E, roles: R) {
	if (!roles || roles.size === 0) throw new PastaError({ message: `We couldn't use any roles for this autorole.` });
...
export function splitComponentsIntoRows(components: any[]) {
	...
	components.forEach((c, i) => {
			...
			if (row.length === 5 || i === components.length - 1) {
					rows.push({	type: ComponentType.ActionRow,	components: row	});
					row = [];
					return;
			}
	});

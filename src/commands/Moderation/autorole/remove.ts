...
export const run: Params = async (interaction, autorole, channel, roles, message) => {
  ...
  if (roles.map((r) => r?.id).some((r: any) => !autorole.role_ids.includes(r))) {
    throw new UserError(`A role provided isn't being used.`);
  }
  ...
  if (rolesRemaining.length == 0) throw new UserError("We cannot remove every role on this autorole message.");
  ...
  if (autorole.type == "button") {
    ...
    if (components.length <= 0) throw new UserError("There needs to be at least one button.");
  ...
  if (autorole.type == "menu") {
    ...
    if (opts.length <= 0) throw new UserError("There needs to be at least one role on the menu.");

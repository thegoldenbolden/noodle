...
export default <Command>{
  ...
  async execute(interaction: ContextMenuCommandInteraction, guild: GuildProfile) {
    ...
    if (!me) throw new UserError(`We were unable to find ourselves. D:`);
				
    if (guild.channels.starboard === null) throw new UserError("A starboard channel isn't setup for this server yet. Use \*settings channels add\* to set up one.");
				const starboard = interaction.guild?.channels?.cache.get(`${guild.channels.starboard}`) as TextChannel;
    if (!starboard) throw new UserError(`We were unable to find the starboard channel.`);
				...
				if (!me.permissionsIn(starboard).has(perms)) throw new UserError(`We need the following permissions in ${starboard}: View Channel, Send Messages, Embed Links, Attach Files`);
    ...
    if (!channel) throw new UserError(`We were unable to find the channel the message is in.`);
				if (channel.id === starboard.id)	throw new UserError(`This message is already in ${starboard}.`);
    if (!me.permissionsIn(channel).has(perms)) throw new UserError("We need the permission \*Add Reactions\* for this command to work.");

    const message = channel.messages.cache.get(interaction.targetId);
    if (!message) throw new UserError("We were unable to find the message.");
    if (message.reactions.cache.get("⭐")?.me) throw new UserError("This message has already been sent to the starboard.");
    await message.react("⭐");

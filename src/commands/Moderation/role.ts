...
import { getColor, split } from "../../utils/functions/helpers";
...
export default <Command>{
 ...
 async execute(interaction: ChatInputCommandInteraction) {
  ...
  async function create(): Promise<string> {
   ...
   if (!created) {
    Errors.send(`Role Create: ${created.message}`);
   ...
   if (user) {
    ...
    if (!added) {
     Errors.send(`Role Create Give: ${created.message}`);
     return `Failed to create role. Reason: \*\*${created.message}\*\*`;
    }
  ...
  async function edit(): Promise<string> {
   ...
   if (!edit) {
    Errors.send(`Role Create: ${edit.message}`);

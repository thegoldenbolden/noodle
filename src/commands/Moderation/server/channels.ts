...
export const run: Run = async (interaction, guild) => {
 ...
 if (channel) {
  ...
  if (!interaction.guild?.members?.me?.permissionsIn(channel.id).has(permissions)) {
   throw new PastaError({ message: `I do not have the View Channel and/or Send Messages permissions for ${channel}` });
  }

  if (channels[type] === channel.id) {
   throw new PastaError({ message: `This channel is already set as the ${type}.` });
  }

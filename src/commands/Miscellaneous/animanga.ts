...
export default <Command>{
 ...
 async execute(interaction: ChatInputCommandInteraction) {
  ...
  const ids: string[] = [...buttons.map((button) => button.custom_id), paginate[page].menu.customId];
  ...
  function setCharacterEmbed(data: any): APIEmbed[] {
   ...
   const description = [
    `${data.about?.substring(0, 2500).trim() ?? "No description available"}${data.about?.length > 2500 ? "..." : ""}`,
   ];
  ...
  function setAnimeEmbed(data?: any): APIEmbed[] {
   ...
   const description = [
    (data?.synopsis?.substring(0, 250) ?? "No synopsis provided").trim() + data?.synopsis?.length > 0 ? "..." : "",
   ];
  ...
  async function getRandom(): Promise<void> {
   ...
   async function get() {
    const { data } = await useAxios({
     name: `Animanga: ${api}random/${display}`,
     url: api + `random/${display}`,
    });
  ...
  async function getSeries(): Promise<Paginate> {
   ...
   if (min && max && min > max) {
    throw new PastaError({ message: `${max} isn't bigger than ${min}.` });
   }

   const missing = (t: "type" | "status" | "order", v: string | null) => {
    if (v === null) return;
    if (!self?.choices[display!][t].some((c: any) => c.value === v)) {
     throw new PastaError({ message: `${display![0].toUpperCase() + display!.substring(1)} does not have ${v} ${t}.` });
    }
   };
  ...
  async function shared(args: any, page: number): Promise<any> {
   ...
   if (!pagination || pagination.items?.total === 0 || data?.length === 0) {
    throw new PastaError({ message: errorMessage });
   }
   ...
   embeds.push(
    ...data.map((result: any, i: number): APIEmbed => {
     ...
     if (!embed[0]) {
      throw new PastaError({ message: "We were unable to create a response.", command: "Animanga", me: true });
     }

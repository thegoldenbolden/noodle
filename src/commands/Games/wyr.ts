...
const Questions: Collection<string, Map<number, boolean>> = new Collection();
export const run = async (interaction: ChatInputCommandInteraction) => {
  let user = Questions.get(`${interaction.user.id}`);

  if (!user) {
    Questions.set(`${interaction.user.id}`, new Map());
    user = Questions.get(`${interaction.user.id}`);
    const timeout = setTimeout(() => {
      Questions.delete(`${interaction.user.id}`);
  ...
  const collector = new InteractionCollector(interaction.client, {
    ...
    filter: (i: ButtonInteraction) =>
      ...
      `WYR-${interaction.user.id}` === i.customId,
  });

...
export function checkSend(channel: TextBasedChannel | null) {
 if (!channel) throw new BotError({ message: "We couldn't find this channel." });
 if (channel.type !== ChannelType.GuildText)
  throw new BotError({ message: "We need to be in a text channel for this command to work." });

 const me = channel?.guild.members?.me;
 if (!me) throw new BotError({ message: "Unable to find my permissions" });
 ...
 if (!me.permissionsIn(channel.id).has(permissions)) throw new BotError({ message: `In ${channel}, ${msg}` });
...
export function checkCooldown(interaction: CommandInteraction, command: Command) {
...
export function checkPermissions(member: CommandInteraction["memberPermissions"], command: Command["permissions"]) {
 if (!command) return;
 if (!member) throw new BotError({ message: "We were unable to check your permissions, cancelling command." });
 if (!member.has(command, true)) throw new BotError({ message: `We require the following permission(s): ${command.join(", ")}` });
}

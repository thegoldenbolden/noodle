...
export async function sendMessage(
  ...
  const user = `${
    message.member?.displayName ?? message.author?.tag ?? "A User"
  }`;
  ...
  const embed: APIEmbed = {
    ...
    title: `${
      (message.channel as TextChannel).nsfw
        ? "This was starred in a NSFW channel"
        : ""
    }`,
    url: `${message.url}`,
    timestamp: new Date(
    ...
    footer: {
      text: `Starred in ${
        (message.channel as TextChannel).nsfw ? "NSFW" : ""
      ...
      if (t === "attachments") {
        ...
        attachment = message.stickers.find(
          (s) =>
            ...
            s.format === StickerFormatType.PNG
        );
      }

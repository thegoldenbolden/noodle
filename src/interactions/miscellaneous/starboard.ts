...
export default {
  ...
  async execute(interaction: Interaction) {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      await interaction.editReply(
        `Whenever a user right clicks a message -> App -> Starboard, the message will be sent to ${created}.  Defaults to users with the 'Manage Guild' permission but can be changed in \`\`Server Settings -> Integrations -> Noodle -> 'Starboard'\`\``
      );
      return;
    }

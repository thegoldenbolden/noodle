...
export function convert(time: Time, options?: Options) {
 ...
 if (options) {
  converted = options.tz && dayjs.extend(tz) ? converted.tz(options.tz) : converted;
  converted = options.utc && dayjs.extend(utc) ? converted.utc(options.utc) : converted;
  converted = options.format && dayjs.extend(parse) ? converted.format(options.format) : converted;
 }
